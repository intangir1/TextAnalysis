...
public class AntonimService extends Application {
    final String TAG = AntonimService.this.getClass().getSimpleName();

    private ProgressDialog progMsg;
    private static AntonimService instance = null;
    private Activity activity;

    private AntonimService(){

    }


    public static AntonimService Get_instance(Activity _activity){
        if(instance == null){
            instance = new AntonimService();
        }
    ...
    public void GetAllWords(CallbacksForRelational _callbacksForRelational){
            ...
            public void onSuccess(String downloadedText, int httpStatusCode) {
                ...
                try {
                    ...
                    for (int i = 0; i < jArray.length(); i++) {
                        ArrayList<String> inner_result = new ArrayList<String>();
                        JSONArray innerjArray = jArray.getJSONArray(i);
                        for (int j = 0; j < innerjArray.length(); j++) {
                            String str = innerjArray.getString(j);
                            inner_result.add(str);
                        }
        ...
        apiGetManager.execute(DataStore.byAntonims);
    }
    ...
    public void GetWordsBy(String word, CallbacksWordsArrayReturn _callbacksWordsArrayReturn){
        ...
        ApiGetManager apiGetManager = new ApiGetManager(new CallbacksWebApi() {
            ...
            public void onSuccess(String downloadedText, int httpStatusCode) {
                ...
                if(httpStatusCode == HttpURLConnection.HTTP_NO_CONTENT){
                    Toast.makeText(instance.activity, "No antonyms were found", Toast.LENGTH_SHORT).show();
                    callbacksWordsArrayReturn.Return_antonims(result);
                } else {
                    JSONArray jArray = null;
                    try {
                        jArray = new JSONArray(downloadedText);
        ...
        apiGetManager.execute(DataStore.byAntonims+word);
    }
}
