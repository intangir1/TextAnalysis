...
public class TempArchaismService extends Application {
    final String TAG = TempArchaismService.this.getClass().getSimpleName();

    private CallbacksForTemporalObjects callbacksForTemporalObjects;
    private ProgressDialog progMsg;
    private Activity activity;
    private static TempArchaismService instance = null;

    private TempArchaismService(){

    }


    public static TempArchaismService Get_instance(CallbacksForTemporalObjects _callbacksForTemporalObjects, Activity _activity){
        if(instance == null){
            instance = new TempArchaismService();
        }
    ...
    public void PostWord(FullText word){
        ...
        postManager.execute(DataStore.byTempArchaism, word.toJSON().toString());
    }
    ...
    public void PutWord(String word_to_replace, FullText word){
        ...
        putManager.execute(DataStore.byTempArchaism + word_to_replace, word.toJSON().toString());
    }
}
