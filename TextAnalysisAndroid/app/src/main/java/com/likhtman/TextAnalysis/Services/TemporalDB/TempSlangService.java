...
public class TempSlangService extends Application {
    final String TAG = TempSlangService.this.getClass().getSimpleName();

    private CallbacksForTemporalObjects callbacksForTemporalObjects;
    private ProgressDialog progMsg;
    private Activity activity;
    private static TempSlangService instance = null;

    private TempSlangService(){

    }


    public static TempSlangService Get_instance(CallbacksForTemporalObjects _callbacksForTemporalObjects, Activity _activity){
        if(instance == null){
            instance = new TempSlangService();
        }
    ...
    public void PostWord(FullText word){
        ...
        postManager.execute(DataStore.byTempSlangs, word.toJSON().toString());
    }
    ...
    public void PutWord(String word_to_replace, FullText word){
        ...
        putManager.execute(DataStore.byTempSlangs + word_to_replace, word.toJSON().toString());
    }
}
